...
uint32_t units_set(GPIO_PinState state,ECUB_Status_t *ECUB_Status) //zapnuti vypnuti napajeni jednotek
	...
	if (pinstate != state){ //pokud se meni pin
		HAL_GPIO_WritePin(ECUA_en_GPIO_Port,ECUA_en_Pin,state); //nastav pin na state
		if (state == SET){
	...
	if (pinstate != state){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(ECUF_en_GPIO_Port,ECUF_en_Pin,state); //nastav pin na state
			if (state == SET){
	...
	if (pinstate != state){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(ECUMCR_en_GPIO_Port,ECUMCR_en_Pin,state); //nastav pin na state
			if (state == SET){
	...
	if (pinstate != state){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(ECUMCF_en_GPIO_Port,ECUMCF_en_Pin,state); //nastav pin na state
			if (state == SET){
	...
	if (pinstate != state){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(EM_en_GPIO_Port,EM_en_Pin,state); //nastav pin na state
			if (state == SET){
...
uint32_t aux_set(GPIO_PinState state,ECUB_Status_t *ECUB_Status) //nastavi napajeni aux
	...
	if (pinstate != state){ //pokud se meni pin
		HAL_GPIO_WritePin(Aux_1_GPIO_Port,Aux_1_Pin,state); //nastav pin na state
		if (state == SET){
	...
	if (pinstate != state){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(Aux_2_GPIO_Port,Aux_2_Pin,state); //nastav pin na state
			if (state == SET){
	...
	if ((pinstate != state)&&(wait_for_next+100<HAL_GetTick())){ //pokud se meni pin
		if(wait_for_next+pauza<HAL_GetTick()){ //pokud ubehlo dost casu od posledni pauzy
			HAL_GPIO_WritePin(Aux_3_GPIO_Port,Aux_3_Pin,state); //nastav pin na state
			if (state == SET){
